...
>
    ...
    <div class='flex min-h-[680px] flex-1 overflow-hidden rounded-2xl border border-slate-100 bg-white'>
        <!-- Sidebar -->
                ...
                <div class='flex flex-col gap-10'>
                    {#each stepsList as step}
                        ...
                        <div class='relative z-10 flex gap-4'>
                            <div class={`flex h-[48px] w-[48px] shrink-0 items-center justify-center rounded-[14px] transition-colors ${
                                isActive
        ...
        <div class='flex flex-1 flex-col overflow-y-auto bg-white px-8 py-14 md:px-[60px] lg:px-[100px]'>
            <div class='mx-auto flex w-full max-w-[600px] flex-col'>
                ...
                {#if currentStep === 1}
                    ...
                    <div class='mt-10 mb-10 rounded-xl border border-[#0A77FF] bg-blue-50/20 p-5'>
                    ...
                    <div class='flex w-full gap-3'>
                        <button
                            ...
                            class='flex flex-1 items-center justify-center rounded-xl border border-slate-200 py-3 text-[13px] font-semibold text-slate-700 transition-colors hover:bg-slate-50'
                        >Back</button>
                        <button
                            onclick={next}
                            class='flex flex-1 items-center justify-center rounded-xl bg-[#0A77FF] py-3 text-[13px] font-semibold text-white transition-colors hover:bg-[#0864d6]'
                        >Continue</button>
                    </div>
                ...
                {:else if currentStep === 2}
                    ...
                    <div class='mt-8 mb-10 grid grid-cols-1 gap-3 sm:grid-cols-2'>
                        {#each mockApp.competencies as comp (comp)}
                            <div class='flex items-center justify-between rounded-xl border border-[#0A77FF] bg-blue-50/30 p-3.5'>
                                <div class='flex items-center gap-3'>
                                    <Hexagon size={15} class='text-[#0A77FF]' strokeWidth={2} />
                                    <span class='text-[12px] font-medium text-slate-600'>{comp}</span>
                                </div>
                                <div class='flex h-4 w-4 shrink-0 items-center justify-center rounded border border-[#0A77FF] bg-[#0A77FF]'>
                    ...
                    <div class='flex w-full gap-3'>
                        <button onclick={back} class='flex flex-1 items-center justify-center rounded-xl border border-slate-200 py-3 text-[13px] font-semibold text-slate-700 transition-colors hover:bg-slate-50'>Back</button>
                    ...
                    <div class='mt-8 mb-10 flex flex-col gap-3'>
                        {#each mockApp.equipments as eq (eq.name)}
                            <div class='flex items-center gap-4 rounded-xl border border-slate-200 bg-white p-4'>
                                <div class='h-14 w-20 shrink-0 overflow-hidden rounded-lg bg-slate-100 flex items-center justify-center text-xs text-slate-400'>
                                    No Image
                                </div>
                                <div class='flex-1'>
                ...
                {:else if currentStep === 4}
                    ...
                    <div class='mt-8 mb-10 flex flex-col gap-3'>
                        {#each mockApp.mouDocs as doc (doc.name)}
                            <div class='flex items-center gap-4 rounded-xl border border-slate-200 bg-white p-4'>
                                <div class={`flex h-10 w-8 shrink-0 items-center justify-center rounded-md text-white text-[10px] font-bold ${iconBg(doc.extension)}`}>
                                    {doc.extension}
                                </div>
                                ...
                                <div class='flex h-6 w-6 items-center justify-center rounded-md bg-[#0A77FF]'>
                                    <Check size={14} class='text-white' strokeWidth={3} />
                                </div>
                            </div>
                        {/each}
                    ...
                    <div class='mt-8 mb-10 grid grid-cols-1 gap-3 sm:grid-cols-2'>
                        {#each mockApp.staff as s (s.position)}
                            <div class='flex flex-col rounded-xl border border-slate-200 bg-white p-4'>
                                ...
                                <p class='mt-2 w-fit rounded bg-slate-50 px-1.5 py-0.5 text-[11px] uppercase tracking-wider text-slate-400'>
                                    Count: {s.quantity}
                                </p>
                            </div>
                        {/each}
