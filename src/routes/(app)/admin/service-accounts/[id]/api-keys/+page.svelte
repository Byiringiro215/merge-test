<script lang='ts'>
    import type { ApiKey, ServiceAccount } from '$lib/datamodel/admin';
    ...
    const apiKeysQuery = $derived(
        fetchServiceAccountApiKeys({ id: serviceAccountId }),
    );
    ...
    $effect(() => {
        if (!apiKeysQuery.loading && !serviceAccountQuery.loading) {
            isInitialLoad = false;
        }
    });
    const showLoading = $derived(
        !isInitialLoad && (apiKeysQuery.loading || serviceAccountQuery.loading),
    );
    ...
    async function handleMint() {
        ...
        isMinting = true;
        try {
            const result = await mintServiceAccountApiKey({
                id: serviceAccountId,
                name: mintName.trim(),
                ...(mintExpiresAt ? { expiresAt: new Date(mintExpiresAt).toISOString() } : {}),
            });
    ...
    const handleRevoke = async () => {
        ...
        revokeError = '';
        try {
            await revokeServiceAccountApiKey({
                id: serviceAccountId,
    ...
    <div class='border px-5 py-6 lg:py-8 lg:px-7'>
        <div class='mb-10'>
            <h1 class='text-[24px] font-bold text-gray-900 leading-tight'>
                API keys: {serviceAccount?.displayName ?? serviceAccount?.name ?? '...'}
            </h1>
            <p class='mt-1 text-sm text-gray-500'>
                Manage API keys owned by this service account. Raw keys are shown
                only once at creation and cannot be retrieved later.
            </p>
        </div>
...
>
    <Dialog.Content class='sm:max-w-xl'>
        <Dialog.Header>
            ...
            <Dialog.Description>
                The raw key will be shown exactly once. Store it immediately — it
...
>
    <Dialog.Content class='sm:max-w-lg'>
        <Dialog.Header>
            ...
            <Dialog.Description>
                <span class='font-medium'>{revealedKeyLabel}</span> — this is the
                only time the raw key will be shown. Copy and store it in a secret
...
<ConfirmDialog
    ...
    title='Revoke API key'
    message='This permanently revokes the key. Any system using it will lose access immediately.'
    confirmLabel='Revoke'
