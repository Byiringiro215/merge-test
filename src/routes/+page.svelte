<script lang="ts">
	...
	let authenticatedApps = $derived(apps.filter((app) => app.isAuthenticated));

	let moreApps = $derived(apps.filter((app) => !app.isAuthenticated));

	let filteredAuthenticatedApps = $derived(
		searchQuery
...
{#snippet AppCard(app: App)}
	<a href={resolve(app.href)} class="block">
		<Card
